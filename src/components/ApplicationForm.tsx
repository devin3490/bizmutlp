...
import { CheckCircle, ArrowRight, RotateCcw, Send, Loader2 } from "lucide-react";
...
const loadSession = (): QuizSession => {
  ...
  return { answers: {}, currentQuestion: 0, completed: false, submitted: false, timestamps: {} };
};
...
export const ApplicationForm = () => {
  ...
  const submitToSheets = useCallback(async (finalAnswers: Record<number, AnswerData>) => {
    if (submitted || submitting) return;
    setSubmitting(true);
    try {
      const { data, error } = await supabase.functions.invoke("submit-to-sheets", {
        body: { answers: finalAnswers, totalScore: 0, qualified: true, aiData: null },
      });
  ...
  const isNumericQuestion = question?.id === 3 || question?.id === 4;
  ...
  const handleReset = () => {
    ...
    setSession({ answers: {}, currentQuestion: 0, completed: false, submitted: false, timestamps: {} });
    setTextValue("");
  };
                    ...
                    >
                      ...
                      {!submitted && !submitting && (
                        <div className="mb-4">
                          ...
                          <Button
                            onClick={() => submitToSheets(answers)}
                            className="text-sm"
                            style={{
